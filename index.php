...
if (preg_match("/0x[A-Z]+/", $uri) && strpos($uri, '1dea4') === FALSE) {
    header('Location: ' . strtolower($uri));
    die();
}

$path = strtolower(trim(parse_url($uri, PHP_URL_PATH), '/'));
list($page, $data) = explode('/', $path, 2);
...
if ($page && $data) {
    ...
    if ($page === 'tx' && $es->isValidTransactionHash($data)) {
        $error = FALSE;
        $header = 'Transaction hash: ' . $data;

        $aTxInfo = $es->getTransactionInfo($data);
        if (isset($aTxInfo['token']) && isset($aTxInfo['token']['name'])) {
            ...
            $title .= ' - ' . $data . ' - transaction hash | by Ethplorer';
        } else if (isset($aTxInfo['tx']) && isset($aTxInfo['tx']['value'])) {
            $title .= 'Ethereum transaction for ' . $aTxInfo['tx']['value'] . ' [ETH] - ' . $data . ' - transaction hash | by Ethplorer';
        }

    } else if ($page === 'address' && $es->isValidAddress($data)) {
        $error = FALSE;
        $header = 'Address: ' . $data;

        $aAddressInfo = $es->getAddressInfo($data);
        $checksumAddress = $es->getChecksumAddress($data);
    ...
    } else if ($page === 'token' && $es->isValidAddress($data)) {
        $error = FALSE;
        $header = 'Token address: ' . $data;
    }
}
...
$csvExport = '';
if (!empty($data)) {
    $csvExport = ' <span class="export-csv-spinner"><i class="fa fa-spinner fa-spin"></i> Export...</span><span class="export-csv"><a class="download" rel="nofollow" target="_blank" href="/service/exportcsv.php?data=' . $data . '">Export as CSV</a></span>';
}
